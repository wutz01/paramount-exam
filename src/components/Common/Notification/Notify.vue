        ...
        <div v-for="item in items" :key="item.id"  :class="'notification notification-'+item.options.type"  @click="removeItem(item.id)">
           <div class="notification-message">
              <div class="row">
                <div class="col-2">
                  <i :class="{
                      'mdi-checkbox-marked-circle': item.options.type === 'success filled',
                      'mdi-alert-circle': item.options.type === 'error filled',
                      'mdi-alert': item.options.type === 'warning filled',
                      'mdi-information-outline': item.options.type === 'info filled'
                    }"
                  class="mdi toast-icon"></i>
                </div>
                <div class="col-10">
                  <h5 class="text-dark  font-weight-normal" v-if="item.title">{{ item.title }}</h5>
                  <p class="text-dark" v-if="item.message" v-html="item.message"></p>
                </div>
              </div>
           </div>
        </div>
